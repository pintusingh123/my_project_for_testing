...
app.get("/notes", async (req, res) => {
  ...
  // find() -> get all documents from the collection in array of object formate if notes null then it return empty array []
...
app.patch("/notes/:id", async (req, res) =>{
  ...
  const note = await noteModel.findOneAndUpdate({_id : id},{description : description})
  res.status(200).json({
    message : "note updated now",
    update: note
  })
})
